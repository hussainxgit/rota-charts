    ...
    const ResidentsApp = {
        ...
        processData() {
            ...
            this.data.residents.forEach(resident => {
                try {
                    resident.startDate = this.parseDate(resident.startDate);
                    resident.endDate = this.parseDate(resident.endDate);
                    ...
                    console.error(`Error processing date for resident: ${resident.name}`, e);
                    // Set default values to prevent errors
                    resident.durationDays = 0;
                    resident.durationMonths = 0;
                    resident.startYear = new Date().getFullYear();
                    resident.startMonth = 0;
                    resident.endYear = new Date().getFullYear();
                    resident.endMonth = 0;
                }
            });
        },
        ...
        generateTableRows() {
            return this.filteredData.map(resident => {
                // Format dates consistently with a fallback
                const startDate = resident.startDate instanceof Date ?
                    resident.startDate.toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' }) :
                    'Invalid Date';

                const endDate = resident.endDate instanceof Date ?
                    resident.endDate.toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' }) :
                    'Invalid Date';
        ...
        renderSummaryStats() {
            ...
            // Average duration
            // In the renderSummaryStats method
            const totalDuration = this.filteredData.reduce((total, resident) => {
                // Ensure we have valid dates before calculating duration
                if (resident.startDate instanceof Date && resident.endDate instanceof Date) {
                    return total + resident.durationDays;
                }
                return total;
            }, 0);
